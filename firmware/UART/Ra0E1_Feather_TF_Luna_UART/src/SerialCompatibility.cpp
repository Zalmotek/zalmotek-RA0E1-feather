...
fsp_err_t SerialCompatibilityLayer::uart_print(uint8_t *p_msg, uint16_t msg_len) {
    ...
    /* Writing to terminal */
    err = R_SAU_UART_Write(&g_uart0_ctrl, p_msg, msg_len);
...
void SerialCompatibilityLayer::uart_init(uint32_t *baud) {
    ...
    assert(err == FSP_SUCCESS);
    sau_uart_baudrate_setting_t SCI_baud_setting;
    uint32_t baud_rate = *baud;
//    bool enable_bitrate_modulation = true;
//    uint32_t error_rate_x_1000 = 5000;
    err = R_SAU_UART_BaudCalculate(&g_uart0_ctrl, baud_rate, &SCI_baud_setting);
    assert(FSP_SUCCESS == err);
    err = R_SAU_UART_BaudSet(&g_uart0_ctrl, (void *) &SCI_baud_setting);
}

void SerialCompatibilityLayer::uart_close(void) {
    fsp_err_t err = R_SAU_UART_Close (&g_uart0_ctrl);
    assert(err == FSP_SUCCESS);
}
